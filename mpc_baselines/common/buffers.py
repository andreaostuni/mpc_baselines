...
class MPCReplayBuffer(BaseBuffer):
    """
    ...
    def sample(self, batch_size: int, env: Optional[VecNormalize] = None) -> MPCReplayBufferSamples:
        """
    ...
    def _get_samples(self, batch_inds: np.ndarray, env: Optional[VecNormalize] = None) -> MPCReplayBufferSamples:
        ...
        data = (
            ...
            self.mpc_states[batch_inds, env_indices, :],
            next_obs,
        ...
        return MPCReplayBufferSamples(*tuple(map(self.to_torch, data)))
...
class MPCDictReplayBuffer(MPCReplayBuffer):
    """
    MPC Dict Replay buffer used in off-policy algorithms like SAC/TD3.
    Extends the MPCReplayBuffer to use dictionary observations
    ...
    def __init__(
        ...
        super(MPCReplayBuffer, self).__init__(buffer_size, observation_space, action_space, mpc_state_dim, device, n_envs=n_envs)

        assert isinstance(self.obs_shape, dict), "MPCDictReplayBuffer must be used with Dict obs space only"
        self.buffer_size = max(buffer_size // n_envs, 1)
        ...
        assert not optimize_memory_usage, "MPCDictReplayBuffer does not support optimize_memory_usage"
        ...
        if psutil is not None:
            ...
            total_memory_usage: float = obs_nbytes + self.actions.nbytes + self.mpc_states.nbytes + self.rewards.nbytes + self.dones.nbytes
            if not optimize_memory_usage:
                next_obs_nbytes = 0
                for _, obs in self.observations.items():
    ...
    def sample(  # type: ignore[override]
        ...
        :return:
        """
        return super(MPCReplayBuffer, self).sample(batch_size=batch_size, env=env)
    ...
    def _get_samples(  # type: ignore[override]
        ...
        return MPCDictReplayBufferSamples(
            observations=observations,
            actions=self.to_torch(self.actions[batch_inds, env_indices]),
            mpc_states=self.to_torch(self.mpc_states[batch_inds, env_indices]),
...
class MPCDictRolloutBuffer(MPCRolloutBuffer):
    ...
    def __init__(
        ...
        assert isinstance(self.obs_shape, dict), "MPCDictRolloutBuffer must be used with Dict obs space only"
    ...
    def reset(self) -> None:
        ...
        self.advantages = np.zeros((self.buffer_size, self.n_envs), dtype=np.float32)
        self.generator_ready = False
        super(MPCRolloutBuffer, self).reset()
