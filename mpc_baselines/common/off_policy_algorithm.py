...
from mpc_baselines.common.buffers import MPCDictReplayBuffer, MPCReplayBuffer
...
SelfMPCOffPolicyAlgorithm = TypeVar("SelfMPCOffPolicyAlgorithm", bound="MPCOffPolicyAlgorithm")


class MPCOffPolicyAlgorithm(BaseAlgorithm):
    """
    The base for Off-Policy algorithms (ex: SAC/TD3) with MPC controller.
    ...
    def __init__(
        ...
        self.replay_buffer: Optional[MPCReplayBuffer] = None
    ...
    def _setup_model(self) -> None:
        ...
        if self.replay_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                self.replay_buffer_class = MPCDictReplayBuffer
            else:
                self.replay_buffer_class = MPCReplayBuffer
    ...
    def load_replay_buffer(
        ...
        assert isinstance(self.replay_buffer, MPCReplayBuffer), "The replay buffer must inherit from MPCReplayBuffer class"
    ...
    def learn(
        self: SelfMPCOffPolicyAlgorithm,
        total_timesteps: int,
        callback: MaybeCallback = None,
        log_interval: int = 4,
        tb_log_name: str = "run",
    ...
    def _sample_action(
        ...
        if self.num_timesteps < learning_starts and not (self.use_sde and self.use_sde_at_warmup):
            ...
            assert self._last_mpc_state is not None, "self._last_mpc_state was not set"
            unscaled_action, _ = self.predict(self._last_obs, self._last_mpc_state, deterministic=False)
    ...
    def _store_transition(
        ...
        mpc_state = deepcopy(self._last_mpc_state)
    ...
    def collect_rollouts(
        ...
        """
        Collect experiences and store them into a ``MPCReplayBuffer``.
    ...
    def predict(
        ...
        :param mpc_state: The MPC controller state
        ...
        """
        return self.policy.predict(observation, mpc_state, state, episode_start, deterministic)
